...
select menu_name,
       ...
       menu_price + menu_price*0.1 as "부가세 포함 전체 가격"
from tbl_menu;

# =============================================================
# ResultSet : SELECT 조회 결과 행의 집하
# 컬럼명 as 별칭 : ResultSet에 표기되는 컬럼명에 별칭(alias) 부여 가능
# =============================================================

select menu_name,
       menu_price as "메뉴 가격",# 쌍 따옴표를 이용해서 띄어 쓰기된 컬럼 묶끼
       menu_price*0.1 부가세, # as 생략
       menu_price + menu_price*0.1 "메뉴 가격"
from tbl_menu;
...
#- 특정 컬럼을기준으로 삼아서 오름차순(ASC), 내림차순(DESC) 지정
#- 기준으로 삼은 컬럼이 여러 개 존재하면 그룹화된 정렬이 수행이 된다.
...
# select 절에 작성되지 않은 컬럼도 정렬기준이 될 수 있다.
select
    menu_name
from tbl_menu
order by
...
/*
메뉴 테이블에서 가격이 10,000원 ㅇ상 , 20000원 이하인
메뉴의 메뉴명 , 가격, 카테고리 코드를
가격 내림차순으로 조회
*/
...
select menu_name,category_code
from tbl_menu
where category_code not in (4,6,10) #4,6,10 제외
order by  category_code ;
...
# where menu_name like '%밥';#밥이 포함된 경우
# where menu_name like '%마늘%';#마늘이 포함된 경우
...
# category 테이블에서 ref_category_code가 null인 행 조회
# select *
# from
#     tbl_category
# where;

#컬럼 안에 null이란 값이 있으면True
...
# limit start, end : start 인덱스 부터 n행 만큼 조회
select * from tbl_menu ;
select * from tbl_menu limit 5;
select * from tbl_menu limit 0, 5;
select * from tbl_menu limit 5, 10;
select * from tbl_menu limit 20, 3; #없는 인덱스를 찾으면 그냥 안나옴
/*
페이징 처리 시 limit를 사용
메뉴 테이블을 이용해서 메뉴판을 만든다고 하였을 떄
한 페이지당 메뉴 4개씩만 조회
순서는 메뉴코드 역순
 */
select * from tbl_menu
