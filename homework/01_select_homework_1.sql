...
/* 2번문제
메뉴명에 '밥'이 포함되고, 가격이 20,000원 이상 30,000원 이하인 메뉴의
메뉴명과 가격을 출력하세요
 */
...
select *
from tbl_menu
where menu_price < 10000 and menu_name like '%김치%'
order by menu_price asc, menu_name desc;

/* 4번문제
 Q1에서 출력한 카테고리명 결과를 참고하여, 카테고리가 기타, 쥬스, 커피에 해당하지 않는 메뉴
   중 메뉴 가격이 13,000원인 메뉴의 모든 컬럼을 출력하세요.
단, 주문이 불가능한 상품은 출력하지 않습니다.

 */
select *
from tbl_menu
where category_code not in (10,9,8) and menu_price = 13000;
