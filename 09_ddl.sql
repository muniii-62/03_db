...
#데이터베이스 스미카(객체)를 만들고(CREATE) 수정(ALTER)하고 삭제(DROP)하는 구문
...
create table product(
    ...
    created_at datetime default current_timestamp comment '상품등록 일시'
);
...
### 제약 조건(constraint)
# 테이블 컬럼에 붙어
# insert, update 시 각 컬럼에 기록되는 값에 대한 조건으 설정하는 방법
...
# not null : 지정된 컬럼은 null일수 없다 == 값 필수
# unique : 지정된 컬럼에는 중복되는 값을 저장할 수 없다.

DROP TABLE IF EXISTS user_notnull;
CREATE TABLE IF NOT EXISTS user_notnull (
    user_no INT NOT NULL,
    user_id VARCHAR(255) NOT NULL, # 컬럼레벨 제약조건 설정
...
(3, 'user03', 'pass03',
 '이순신', '남',
 '010-777-7784', 'lee222@gmail.com');

select * from user_unique

#primary key (중요)
# 테이블 내 행을 구별하기 위한 식별자 역할의 컬럼에 추가하는 제약조건
# not null + unique : 중복 되지 않는 값 필수
#primary key는 테이블 내에 1개만 설정 가능
#단, pk 설정이 적용되는 컬럼은 1개 또는 여러 개 묶음(복합 키 ) 가능
...
#  Duplicate entry '3-100-2026-06-12 11:31:27' for key 'order_composite_pk.PRIMARY'



### FOREIGN KEY(외래키)
# - 참조(REFERENCE)된 다른 테이브에서 제공하는갓만 사용 가능
# - 참조 무결석을 위배하지 않기 위해 사용한다.
# - 다른 테이블의 pk 또는 unique가 설정된 컬럼만 참조가능하다.
# fk 제약조건 설정 시 두 테이블간의 관계(relationshiop)이 형성된다.
...
#  부모 컬럼 값을 자식이 참조중이면 변경, 삭제 시
# 자식 컬럼 값을 null로 변경

# -update/delete cascade :
#  부모 컬럼 값을 자식이 참조중이면 변경, 삭제 시
# 자식 테이블엥서 참조 값이 포함된 모든 행을 삭제 (연쇄적)
...
#컬럼 추가
alter table product
add description varchar(25) not null
default '설명 없음'
after price;
